...
result.step<-step(model.All.coxph, 
                  scope = list(upper = ~Race+Age+ER+PR+HER2+Grade+Stage+AveTCount+AveCMCount+AveNCount, 
                               lower = ~AveTCount)) #forcing AveTCount in the model 
result.step 
...
```{r}
#result.step 
coef.est <- c(0.044207, NA, NA, NA,0, 0.433748, NA, NA, NA, 0, 0.605995, NA, NA,NA,0,1.200739, NA,0.841978)
se.est <- c(0.007857, NA, NA,NA, 0, 0.283465, NA, NA, NA, 0, 0.230007,NA, NA, NA,0,0.225934,NA, 0.570057)
lower <- coef.est - 1.96*se.est
upper <- coef.est + 1.96*se.est
label.factors <- matrix(c(" Age","", "HER2","", " negative", " positive", "", "Grade", "",   "  low", " high", "", "Stage", "", "  low", " high", "", "AveNCount"), ncol=1)
```

```{r}
forestplot(label.factors, coef.est, lower=lower, upper=upper,
boxsize=0.4, xticks=c(-0.5,0,0.5, 1, 1.5, 2),
txt_gp=fpTxtGp(label=gpar(cex=1)), xlab="log hazard ratio",
